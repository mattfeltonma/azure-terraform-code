variable "address_space_amlcpt" {
  description = "The CIDR blocks where the AML compute instances and clusters will be deployed"
  type        = list(string)
}

variable "address_space_apim" {
  description = "The CIDR blocks where the internally-facing APIM instances are deployed to"
  type        = list(string)
}
