...
output "subnet_id_bastion" {
  value       = try(azurerm_subnet.subnet_bastion[0].id, null)
  description = "The resource id of the AzureBastionSubnet"
}

output "subnet_name_bastion" {
  value       = try(azurerm_subnet.subnet_bastion[0].name, null)
  description = "The resource name of the AzureBastionSubnet"
}
