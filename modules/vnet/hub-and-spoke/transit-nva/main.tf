## Create transit virtual network and configure its diagnostic settings
##
...
# The public NSG is set initially with no rules which will block all incoming traffic from the Internet
#
...
# The private NSG is set to allow all RFC1918 inbound traffic
#
...
## Create virtual network subnets with a GatewaySubnet, public subnet, private subnet, and subnet for Azure Bastion
##
...
resource "azurerm_subnet" "subnet_bastion" {
  ...
  count = var.subnet_cidr_bastion != null ? 1 : 0
...
resource "azurerm_subnet_network_security_group_association" "nsg_association_firewall_public" {
  depends_on = [
    ...
    azurerm_subnet.subnet_gateway,
    azurerm_subnet.subnet_firewall_private,
    azurerm_subnet.subnet_bastion[0]
  ]
...
## Create an Azure Bastion instance if required
##
module "bastion" {
  depends_on = [
...
# Create an external load balancer in the public subnet
#
module "elb" {
  depends_on = [
    azurerm_subnet.subnet_firewall_public,
...
# Create an internal load balancer in the private subnet
#
module "ilb" {
  depends_on = [
    azurerm_subnet.subnet_firewall_private,
...
## Create route tables for each of the subnets
##

# Create a subnet for the GatewaySubnet which forwards traffic for the Shared Services and Workload subnets
#
...
# Create a route table for private subnet that blackholes traffic to the public subnet and forward all other traffic
# to the internal load balancer to ensure traffic cannot circumvent inspection
...
# Create a route table for public subnet that blackholes traffic to the private subnet and directly attached virtual networks
#
...
## Create the a dummy network virtual appliance which is a Linux VM running Unbuntu and using the vlan driver to allow for usage of
## vlans and uses iptables for filtering traffic
module "nva" {
  depends_on = [
    module.elb,
    module.ilb,
