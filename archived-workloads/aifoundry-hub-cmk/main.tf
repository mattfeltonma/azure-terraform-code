...
# or project.
resource "azurerm_application_insights" "appins_aifoundry" {
  depends_on = [
...
##### Create optional resources for the AI Foundry Hub
#####
...
# Create an Azure AI Services instance. Public access will be disabled with the Trusted Services Exception to allow
# AI Foundry and AI Search access to the Azure OpenAI Service. AI Search will use the Azure OpenAI Service when ingesting data
# and creating vector embeddings. The module will automatically create deployments for GPT-4o and text-embedding-3-large models.
...
##### Create the user-assigned managed identity for the AI Foundry Hub and grant it
##### the necessary role assignments

# Create the user-assigned managed identity for the AI Foundry hub
#
resource "azurerm_user_assigned_identity" "umi_foundry_hub" {
  depends_on = [
...
#
resource "time_sleep" "wait_umi_foundry_hub_creation" {
  depends_on = [
    azurerm_user_assigned_identity.umi_foundry_hub
  ]
  create_duration = "10s"
}

##### Create the resources required to support CMK for AI Foundry and AI Services
#####

# Create Key Vault which will hold the key used for the key for CMK of the Foundry instance. Add access policies (required due to no RBAC support at this time for this) to allow 
...
module "keyvault_cmk" {
  ...
  # Purge protection is required to be enabled to use the CMK feature
  purge_protection = true

  # The user specified here will have the Azure RBAC Key Vault Administrator role over the Azure Key Vault instance, but
  # this is not used because the Key Vault must use access policies when using a user-assigned managed identity.
  kv_admin_object_id = var.user_object_id

  # Disable Azure RBAC and use legacy access policies due to limitation with this integration
  rbac_enabled       = false

  # Create the Key Vault access policies
  access_policies = [
    
    # Grant the identity deploying the Terraform full permissions on the Azure Key Vault instance
    # used to store the CMK
    {
    ...
    # Grant the AI Foundry Hub managed identity the required permissions to use keys stored in the
    # Key Vault
    {
      object_id = azurerm_user_assigned_identity.umi_foundry_hub.principal_id
      secret_permissions = [
      ],
      key_permissions = [
        "Get",
    ...
    # Grant the AI Services managed identity the required permissions to use keys stored in the
    # Key Vault.
    {
      object_id = module.ai_services.managed_identity_principal_id
      secret_permissions = [
      ],
      key_permissions = [
        "Get",
  ...
  # Add the IP address of the machine running Terraform to the Key Vault firewall rules
  # because it is required for redeployments when it accesses the data plane.
  firewall_ip_rules = [
    var.tf_ip_address
  ]
}
...
resource "azurerm_key_vault_key" "key_cmk_foundry" {
  ...
  name         = "cmk-foundry"
  key_vault_id = module.keyvault_cmk.id
  key_type     = "RSA"
...
# Create the CMK used to encrypt the AI Services instance. As of 4/29/2025 the AI Services
# CMK must be 2048 bits.
# https://learn.microsoft.com/en-us/azure/ai-foundry/concepts/encryption-keys-portal#use-customer-managed-keys-with-azure-key-vault
resource "azurerm_key_vault_key" "key_cmk_ai_services" {
  depends_on = [
    resource.azurerm_key_vault_key.key_cmk_foundry
  ]
...
# Modify the Azure AI Services workspace to use the CMK for encryption. This is required 
# to be done after the AI Services instance is created because the system-managed identity must first exist.
#
resource "azapi_resource" "update_ai_services_cmk" {
  depends_on = [
    azurerm_key_vault_key.key_cmk_ai_services,
    module.ai_services_aifoundry
  ]

  type                      = "Microsoft.MachineLearningServices/workspaces@2024-10-01-preview"
  name                      = module.ai_services_aifoundry.name
  ...
  body = {
    properties = {
      encryption = {
        status = "Enabled"
        keyVaultProperties = {
          keyVaultArmId = module.keyvault_cmk.id
          keyIdentifier = azurerm_key_vault_key.key_cmk_ai_services.id
        }
      }
    }
  }

  # Do not modify these tags on redeployment of these templates
  lifecycle {
    ignore_changes = [
      tags["created_date"],
      tags["created_by"]
...
##### Create the non-human role assignments for the AI Foundry Hub identity
#####
...
resource "azurerm_role_assignment" "umi_rg_aiadministrator" {
  depends_on = [
    azapi_resource.foundry_hub.update_ai_services_cmk
  ]

  name                 = uuidv5("dns", "${azurerm_resource_group.rgwork.name}${azurerm_user_assigned_identity.umi_foundry_hub.name}aiadmin")
  scope                = azurerm_resource_group.rgwork.id
  role_definition_name = "Azure AI Administrator"
  principal_id         = azurerm_user_assigned_identity.umi_foundry_hub.principal_id
}

# Create Azure RBAC Role assignments Assign Blob Data Contributor and File Data Privileged Contributor roles to the managed identity for 
# the AI Foundry Hub to the default storage account. This enables the AI Foundry Hub to create necessary 
# blob containers and file shares required by downstream projects
resource "azurerm_role_assignment" "umi_aifoundry_st_blob_data_contributor" {
  depends_on = [
    azurerm_role_assignment.umi_rg_aiadministrator
  ]

  name                 = uuidv5("dns", "${azurerm_resource_group.rgwork.name}${module.storage_account_ai_foundry.name}${azurerm_user_assigned_identity.umi_foundry_hub.name}blobdata")
...
resource "azurerm_role_assignment" "umi_aifoundry_st_file_data_contributor" {
  ...
  name                 = uuidv5("dns", "${azurerm_resource_group.rgwork.name}${module.storage_account_ai_foundry.name}${azurerm_user_assigned_identity.umi_foundry_hub.name}filedata")
  scope                = module.storage_account_ai_foundry.id
  role_definition_name = "Storage File Data Privileged Contributor"
  principal_id         = azurerm_user_assigned_identity.umi_foundry_hub.principal_id
}

# Create role assignment on default Key Vault to allow managed identity to access the Key Vault when 
# needed to read and write secrets required by the workspace and connections
resource "azurerm_role_assignment" "umi_data_kv_admin" {
  depends_on = [
    azurerm_role_assignment.umi_aifoundry_st_file_data_contributor
  ]

  name                 = uuidv5("dns", "${azurerm_resource_group.rgwork.name}${module.keyvault_aifoundry.name}${azurerm_user_assigned_identity.umi_foundry_hub.name}kvadmin")
  scope                = module.keyvault_aifoundry.id
  role_definition_name = "Key Vault Administrator"
  principal_id         = azurerm_user_assigned_identity.umi_foundry_hub.principal_id
}

# Create role assignment for the Foundry Hub managed identity on the resource group containing the services that 
# will have private endpoints created in the managed virtual network. This is required for the hub to create
# the managed private endpoints
resource "azurerm_role_assignment" "umi_rg_azure_ai_ent_net_conn_app" {
  depends_on = [
    azurerm_role_assignment.umi_data_kv_admin
  ]

  name                 = uuidv5("dns", "${azurerm_resource_group.rgwork.name}${azurerm_resource_group.rgwork.name}${azurerm_user_assigned_identity.umi_foundry_hub.name}entnetconnapp")
  scope                = azurerm_resource_group.rgwork.id
...
##### Create the AI Foundry Hub and its child resources
#####
...
# FQDN-based rules. Managed private endpoints for the Azure OpenAI Service, Azure AI Services, Azure AI Search, and the blob and file endopints of the 
# data storage account will be created.

# For identity controls, the hub will use a user-assigned managed identity which has been manually granted the permissions covered in the earlier
# part of the template

# For encryption controls, the AI Foundry Hub will use a CMK.
  ...
  body = {
    identity = {
      ...
      userAssignedIdentities = {
        "${azurerm_user_assigned_identity.umi_foundry_hub.id}" = {}
      }
    }
    ...
    properties = {
      ...
      applicationInsights = azurerm_application_insights.appins_aifoundry.id
      keyVault            = module.keyvault_aifoundry.id
      storageAccount      = module.storage_account_ai_foundry.id
      containerRegistry   = module.container_registry_aifoundry.id

      # Encrypt the AI Foundry Hub with a CMK
      encryption = {
        status = "Enabled"
        keyVaultProperties = {
          keyVaultArmId = module.keyvault_cmk.id
          keyIdentifier = azurerm_key_vault_key.key_cmk_foundry.id
        }
      }
      ...
      managedNetwork = {
        ...
        outboundRules = {
          ...
          managed-pe-aiservice = {
            type = "PrivateEndpoint"
            destination = {
              serviceResourceId = module.ai_services.id
              subresourceTarget = "account"
            }
          }
      ...
      # Do not allow any platform-created RBAC assignments on the resource group because the AI Foundry hub is using a
      # user-assigned managed identity
      allowRoleAssignmentOnRg = false

      # Specify that the user-assigned managed identity should be used
      primaryUserAssignedIdentity = azurerm_user_assigned_identity.umi_foundry_hub.id
...
#
resource "azapi_resource" "workspace_ai_service_connection" {
  depends_on = [
    module.ai_services,
    azapi_resource.foundry_hub
  ]
  ...
  type                      = "Microsoft.MachineLearningServices/workspaces/connections@2024-04-01-preview"
  name                      = "conn${module.ai_services.name}"
  parent_id                 = azapi_resource.foundry_hub.id
  schema_validation_enabled = true
  ...
  body = {
    properties = {
      ...
      category      = "AIServices"
      isSharedToAll = true
      target        = "https://${module.ai_services.custom_subdomain}.openai.azure.com"
      metadata = {
        ApiType    = "Azure"
        ApiVersion = "2024-10-21"
        Location   = local.openai_region
        ResourceId = module.ai_services.id
      }
    }
  }
}
...
# Create Private Endpoints for the data storage account
#
module "private_endpoint_st_data_blob" {
  depends_on = [
    module.private_endpoint_container_registry
...
module "private_endpoint_ai_service" {
  ...
  resource_name    = module.ai_services.name
  resource_id      = module.ai_services.id
  subresource_name = "account"
...
# Create a Private Endpoints for the Key Vault instance that will host the CMK
#
module "private_endpoint_kv_cmk" {
  ...
  resource_name    = module.keyvault_cmk.name
  resource_id      = module.keyvault_cmk.id
  subresource_name = "vault"
...
# Create role assignments granting Azure OpenAI Service and AI Service system-managed identities Search Data Index Reader and Reader role on the AI Search instance.
...
resource "azurerm_role_assignment" "aisearch_reader_openai_mi" {
  ...
  name                 = uuidv5("dns", "${azurerm_resource_group.rgwork.name}${module.openai_aifoundry.managed_identity_principal_id}${module.ai_search.name}reader")
  scope                = module.ai_search.id
...
resource "azurerm_role_assignment" "aisearch_reader_ai_services_mi" {
  ...
  name                 = uuidv5("dns", "${azurerm_resource_group.rgwork.name}${module.ai_services_aifoundry.managed_identity_principal_id}${module.ai_search.name}reader")
  scope                = module.ai_search.id
  role_definition_name = "Reader"
  principal_id         = module.ai_services_aifoundry.managed_identity_principal_id
}

# Create role assignment granting the AI Search instance Congitive Services OpenAI Contributor role
# over Azure OpenAI Service and AI Services instances
...
# for the default storage account and data storage account. This is required for uploaded data for fine-tuning or using the on your own data feature
# of the Chat Playground
