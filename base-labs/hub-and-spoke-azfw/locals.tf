locals {
  ...
  vnet_cidr_wl1_pri = cidrsubnet(var.address_space_azure_primary_region, 2, 2)
  vnet_cidr_wl2_pri = cidrsubnet(var.address_space_azure_primary_region, 2, 3)
  ...
  vnet_cidr_wl1_sec = var.multi_region == true ? try(cidrsubnet(var.address_space_azure_secondary_region, 2, 2), null) : null
  vnet_cidr_wl2_sec = var.multi_region == true ? try(cidrsubnet(var.address_space_azure_secondary_region, 2, 3), null) : null
  ...
  acr_private_dns_namespace_primary   = "${var.location_primary}.data.privatelink.azurecr.io"
  acr_private_dns_namespace_secondary = var.multi_region == true ? ["${var.location_secondary}.data.privatelink.azurecr.io"] : []

  # Add regional zones to a list 
  regional_private_dns_namespaces = concat(
    [
      local.aks_private_dns_namespace_primary,
      local.acr_private_dns_namespace_primary
    ],
    local.aks_private_dns_namespace_secondary,
    local.acr_private_dns_namespace_secondary
  )
