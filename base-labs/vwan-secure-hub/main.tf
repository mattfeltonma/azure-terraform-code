...
module "firewall-pri" {
  ...
  address_space_amlcpt     = cidrsubnet(local.vnet_cidr_wl_pri, 3, 5)
...
module "firewall-sec" {
  ...
  address_space_amlcpt     = cidrsubnet(local.vnet_cidr_wl_sec, 3, 5)
  address_space_azure      = var.address_space_cloud
  address_space_onpremises = var.address_space_onpremises
  sku_name                 = "AZFW_Hub"
...
module "workload-vnet-pri" {
  ...
  subnet_cidr_amlcpt   = cidrsubnet(local.vnet_cidr_wl_pri, 3, 5)
  subnet_cidr_mgmt   = cidrsubnet(local.vnet_cidr_wl_pri, 3, 6)
  subnet_cidr_vint   = cidrsubnet(local.vnet_cidr_wl_pri, 3, 7)
  dns_servers = [
    module.firewall-pri.private_ip_vwan_hub
  ]
...
module "workload-vnet-sec" {
  ...
  subnet_cidr_amlcpt   = cidrsubnet(local.vnet_cidr_wl_sec, 3, 5)
  subnet_cidr_mgmt   = cidrsubnet(local.vnet_cidr_wl_sec, 3, 6)
  subnet_cidr_vint   = cidrsubnet(local.vnet_cidr_wl_sec, 3, 7)
